...
def _process_notes(
    ...
    *,
    clarifying_notes: str | None,
    clarification_questions_useful: str,
) -> str:
    if clarifying_notes and clarifying_notes.strip():
        return clarifying_notes.strip()
    useful_phrase = (
        "Clarifying questions felt useful."
        if clarification_questions_useful == "true"
        else "Clarifying questions did not feel useful."
    )
    path_phrase = {
        "clarify_then_resolve": (
            "The assistant asked focused questions before offering support the "
            "persona considered sufficient."
        ),
        "direct_resolution": (
            "The assistant addressed the support goal directly without an extended "
            "clarification phase."
        ),
        "clarify_then_partial": (
            "The assistant asked clarifying questions and offered support, but the "
            "persona reported meaningful gaps at the end."
        ),
        "stalled": (
    ...
    return f"{path_phrase} {useful_phrase}"
...
def test_transcript_schema() -> None:
    ...
    clarifying_notes_raw = feedback.get("clarifyingNotes")
    clarifying_notes = (
        clarifying_notes_raw.strip()
        if isinstance(clarifying_notes_raw, str) and clarifying_notes_raw.strip()
        else None
    )
    ...
    process_notes = _process_notes(
        conversation_path,
        clarifying_notes=clarifying_notes,
        clarification_questions_useful=clarification_questions_useful,
    )
